...
// used by readModal in sheets.js
...
async function readSheet(url){
  ...
  returnData['columnNames'] = Object.keys( data.entry[0]).filter( col => { 
    return col.toLowerCase().indexOf("gsx$") >= 0 
  });
  ...
  for (let index in data.entry){
    ...
    returnData['columnNames'].map( (column) => {
