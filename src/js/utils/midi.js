...
export async function midi( midiCallback=false){
    var midi_TotalTrack = [
    ...
    // Request MIDI access
    if(navigator.requestMIDIAccess){navigator.requestMIDIAccess({sysex: false}).then(onMIDISuccess, false );}

    // Dispaly MIDI Channels Plugged in
    ...
    // Show new devices on Midi Channels Change...
    ...
    function onMIDIMessage(event){
        ...
        type = event.data[0] & 0xf0, // channel agnostic message type.
        // Type = 144 for pushbuttons
        // Type = 176 for everything else
        note = event.data[1],          
        velocity = event.data[2];
        // console.log('cmd : ' + cmd + ', Type : ', type, ' Map Value : ' + note + ', velocity : ' + velocity);
        
        let obj = midi_TotalTrack.find(obj => obj.key == note);
        console.log('Midi message: ', obj);
