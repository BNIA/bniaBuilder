  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    if( records ){
      ...
      Object.keys(mapLayers).map( mapKey => {
        ...
        if (!layerData){
          //delete mapLayers[mapKey];
          map.removeLayer(mapLayers[mapKey]);
          this.setState( [mapKey] : false );
        }
        else{
      ...
      // Then loop through through our layers
      await Object.keys(records).map( async key => {
        // If the layer is in our state, the records may have been changed. but either way add the legend content.
        if(this.state[key]){
          // check if it should be displayed then append its Legend information. 
          let layerState = this.state[key];
          !layerState.display ? '' : legend.innerHTML += addLayerToLegend( layerState.styles, layerState.alias);
        }
        else{
