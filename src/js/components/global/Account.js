...
import {SimpleDetails, downloadCsv, downloadObjectAsJson} from 'js/utils/utils';
...
export default class Account extends Component {
  ...
  async getSheets(event){
  	const configHandler = await import(/* webpackChunkName: "getSheets" */ 'js/utils/configHandler');
  	event.preventDefault();
    let dropdowns = event.target.getElementsByTagName('select'); 
    // 1) { getSheets } from 'js/utils/sheets'
    let resp = await configHandler.getSheets( dropdowns[0].value );
    ...
    if( resp != null){
      ...
      jsonObject.dictionaries = await configHandler.fillDictionaries(jsonObject.layers);;
  ...
  render() {
    ...
    return ( 
      < div className = "modal_big_wrapper" style={ { display: state.activeModal == "Login" ? "block" : 'none' } } onClick={ turnOff } >
		< section className='modal_big_content'>
          {innerContent}
          <button className = "toggle_view close_account_modal" tabIndex = "0" onClick={ turnOff } > OK < /button> 
		< / section >
      < /div >
    ) 
  }
